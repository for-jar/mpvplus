...
class PlayerPreferences(
  ...
  val holdForMultipleSpeed = preferenceStore.getFloat("hold_for_multiple_speed", 6f)
  ...
  val speedPresets =
    preferenceStore.getStringSet(
      ...
      setOf("0.25", "0.5", "0.75", "1.0", "1.25", "1.5", "1.75", "2.0", "2.5", "3.0", "3.5", "4.0", "4.5", "5.0", "6.0",
"7.0", "8.0"),
    )
  ...
  // -------------------------------------------------------------------------
  // One-shot migration: bump legacy installs to the new 8.0x speed presets
  // and the 6.0x long-press default. Runs exactly once per install.
  // -------------------------------------------------------------------------
  init {
    val migrationFlag = preferenceStore.getBoolean("pref_migration_v2_speed_8x_done", false)
    if (!migrationFlag.get()) {
      // 1) Speed presets: if the stored set looks like the OLD default
      //    (no "8.0" entry), upgrade to the NEW default. If the user has
      //    customised their presets in any way that already includes 8.0,
      //    leave them alone.
      val currentPresets = speedPresets.get()
      if ("8.0" !in currentPresets) {
        speedPresets.set(
          setOf(
      ...
      // 2) Long-press speed: only upgrade if the user is still on the
      //    legacy default of 2f. If they've already picked a custom value
      //    (e.g. 4f), respect it.
      if (holdForMultipleSpeed.get() == 2f) {
        holdForMultipleSpeed.set(6f)
      }
